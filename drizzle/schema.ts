...
/**
 * Table to store gift selections for the wedding registry
 */
export const giftSelections = mysqlTable("giftSelections", {
  id: int("id").autoincrement().primaryKey(),
  giftId: varchar("giftId", { length: 64 }).notNull(),
  giftName: varchar("giftName", { length: 255 }).notNull(),
  selectedBy: varchar("selectedBy", { length: 255 }).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
});
